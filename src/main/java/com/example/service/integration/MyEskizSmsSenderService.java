...
import com.example.entity.sms.SmsTokenEntity;
import com.example.repository.sms.SmsTokenRepository;
...
public class MyEskizSmsSenderService implements SmsSenderService {
    ...
    public void sendSmsHTTP(SmsHistoryEntity smsHistory) {
        ...
        RequestBody body = new MultipartBody.Builder().setType(MultipartBody.FORM)
                ...
                .addFormDataPart("message", smsHistory.getSmsText())
    ...
    public String getSmsToken() {
        ...
        if (smsToken.getCreatedDate().plusDays(25).isAfter(LocalDate.now())) {
            ...
            String token = myEskizUzLogin();
            smsTokenRepository.updateToken(token, email); // update token
            return token;
           /* OkHttpClient client = new OkHttpClient().newBuilder()
            ...
            try {
                ...
                if (!response.isSuccessful()) {
                    ...
                    smsTokenRepository.updateToken(smsToken.getToken(), email); // update token
                    return smsToken.getToken();
                }
            } catch (IOException e) {
                e.printStackTrace();
